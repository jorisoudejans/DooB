...
public class LevelFactory {
	 ...
	 *
	 * @param s
	 *            The XML-tag to parse from
	 * @param eElement
	 *            The XML-Element to parse from
	 * @return The parsed int
	 */
	private int parseInt(String s, Element eElement) {
		return Integer.parseInt(eElement.getElementsByTagName(s).item(0)
				.getTextContent());
	...
	private void parsePlayers(Document doc) {
		...
		for (int i = 0; i < nListPlayer.getLength(); i++) {
			...
			if (nNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				Player pl = new Player(x, y, width, height, playerImages[0],
						playerImages[1], playerImages[2]);
				playerList.add(pl);
			}
		}
	}
	 ...
	 * 
	 * @param i
	 *            Images for the i-th player
	 */
	private void setPlayerImages(int i) {
		playerImages = new Image[] {
				new Image("/image/character" + i + "_stand.png"),
